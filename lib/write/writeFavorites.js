...
var getUserId = function (userUrl, cb) {
  ...
  request(util.createResolveUrl(userUrl), function (err, res) {
    if (err) {
...
var writeTrackUrls = function (tracksUrl, fp, allDone) {
  ...
  var fetchMore = function (tracks, fetchDone) {
    ...
    if (tracks.next_href) {
      var url = tracks.next_href.slice(0, tracks.next_href.length - 2) + '200';
      logger.info('Fetching more tracks at', url);
